...
Read set (allowed for the reader): `GET /heartbeat`, `GET /version`, `GET …/pre-flight-checks`, `GET …/tenants/{t}`, `GET …/databases(/{db})`, `GET …/collections` (list), `GET …/collections/{id}`, `GET …/collections/{id}/count`, `POST …/collections/{id}/query`, `POST …/collections/{id}/get`. Everything else — create/delete collections, `add`/`update`/`upsert`/`delete`, `fork`, tenant/database creation, and `reset` — is a write.
